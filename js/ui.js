    ...
    if (rol === 'Estudiante') {
        return `
            <div class="card">
                <h3>Mi Perfil</h3>
                <div class="grid">
                    <div><label>Nombre</label><input value="${usuario.nombre_completo || ''}" readonly></div>
                    <div><label>Curso</label><input value="${usuario.curso || ''}" readonly></div>
                    <div><label>ID</label><input value="${usuario.id_sistema || ''}" readonly></div>
                </div>
                <div class="grid">
                    <div><label>Padre</label><input value="${usuario.nombre_padre || ''} (${usuario.tel_padre || 'S/N'})" readonly></div>
                    <div><label>Madre</label><input value="${usuario.nombre_madre || ''} (${usuario.tel_madre || 'S/N'})" readonly></div>
                </div>
            </div>`;
    } else {
        // Docente, Coordinación, Orientación
        return `
            <div class="card">
                <h3>Mi Perfil</h3>
                <div class="grid">
                    <div><label>Nombre</label><input value="${usuario.nombre_completo || ''}" readonly></div>
                    <div><label>Cédula</label><input value="${usuario.cedula || ''}" readonly></div>
                    <div><label>Teléfono</label><input value="${usuario.telefono || ''}" readonly></div>
                    <div><label>Email</label><input value="${usuario.email || ''}" readonly></div>
                    <div><label>Dirección</label><input value="${usuario.direccion || ''}" readonly></div>
...
export function renderExcusas(excusas) {
    ...
    if (activas.length) {
        html += `<table class="web-table"><thead><tr><th>Estudiante</th><th>Desde</th><th>Hasta</th><th>Motivo</th></tr></thead><tbody>`;
        activas.forEach(e => html += `<tr><td>${e.nombre}</td><td>${new Date(e.desde).toLocaleDateString()}</td><td>${new Date(e.hasta).toLocaleDateString()}</td><td>${e.motivo}</td></tr>`);
        html += `</tbody></table>`;
    } else {
        html += `<p style="color:var(--text-light); margin-bottom:20px;">No hay excusas activas.</p>`;
    }
    ...
    if (inactivas.length) {
        html += `<table class="web-table" style="opacity:0.7"><thead><tr><th>Estudiante</th><th>Desde</th><th>Hasta</th><th>Motivo</th></tr></thead><tbody>`;
        inactivas.forEach(e => html += `<tr><td>${e.nombre}</td><td>${new Date(e.desde).toLocaleDateString()}</td><td>${new Date(e.hasta).toLocaleDateString()}</td><td>${e.motivo}</td></tr>`);
        html += `</tbody></table>`;
    } else {
        html += `<p style="color:var(--text-light)">No hay historial.</p>`;
    }
...
export function renderListado(alumnos) {
    return `<button onclick="window.print()" class="btn-primary no-print" style="margin-bottom:15px">IMPRIMIR</button>
        <table class="hoja-cuadriculada"><thead><tr><th>#</th><th>Nombre</th><th>Firma</th></tr></thead><tbody>
        ${alumnos.map((a,i) => `<tr><td>${i+1}</td><td>${a.nombre}</td><td></td></tr>`).join('')}
        </tbody></table>`;
}
